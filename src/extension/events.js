...
(function() {
    ...
    show_timeline = function(ds_name, event_) {
        ...
        $.each(event_, function(id, e){
            var events = Events.events[ds_name][e],
                text;

            if (e === "newcomers"){
                text = "Has become a new member";
                $.each(events.date, function(index){
                    data.events.push({"uid":events.uid[index],
            ...
            }else if (e === "goneauthors"){
                text = "Marked as inactive";
                $.each(events.last_activity, function(index){
                    data.events.push({"uid":events.uuid[index],
                                      "name":events.name[index],
                                      "email":events.email[index],
                                      "timestamp":events.last_activity[index],
                                      "event_text": text,
                                      "goneauthors": true});
                });
            }
